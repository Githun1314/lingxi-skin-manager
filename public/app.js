const presets = [
  ...
  { styleId: "game_peace", series: "游戏 IP 概念", name: "和平精英·战术值守", brandName:"精英", description: "冷钢灰、战术蓝、头盔守边", primary:"#c78b2f", background:"#eef2f5", sidebar:"#dfe6ea", card:"#fbfcfd", text:"#202b33", darkPrimary:"#f1b84b", darkBackground:"#11181d", darkSidebar:"#172127", darkCard:"#202c33", darkText:"#edf3f6", radius:12, asset:"/themes/game-peace-scene.svg", brandAsset:"/themes/game-peace-logo.svg", hangerAsset:"/themes/game-peace-hanger.png", backgroundScope:"content", backgroundFit:"cover", backgroundPosition:"right", backgroundOverlay:.66, backgroundBlur:0, composerDecorationEnabled:true, composerDecorationSource:"hanger", composerAnchor:"right-top", composerConversationAnchor:"right-top", composerScale:.22, composerOverlap:.08, decorationOpacity:.98 },
  { styleId: "game_goose", series: "游戏 IP 概念", name: "鹅鸭杀·侦探鹅", brandName:"侦探鹅", description: "侦探紫、奶油白、呆萌探头", primary:"#7058bc", background:"#f7f4ff", sidebar:"#e9e3f8", card:"#fffefe", text:"#2c263c", darkPrimary:"#aa91ef", darkBackground:"#17131f", darkSidebar:"#20192c", darkCard:"#2b2339", darkText:"#f5efff", radius:22, asset:"/themes/game-goose-scene.svg", brandAsset:"/themes/game-goose-logo.svg", hangerAsset:"/themes/game-goose-hanger.png", backgroundScope:"content", backgroundFit:"cover", backgroundPosition:"right", backgroundOverlay:.69, backgroundBlur:0, composerDecorationEnabled:true, composerDecorationSource:"hanger", composerAnchor:"right-top", composerConversationAnchor:"right-top", composerScale:.19, composerOverlap:.08, decorationOpacity:.99 },
  { styleId: "xoyo_chicken", series: "游戏 IP 概念", name: "西山居黄鸡·笑场", brandName:"黄鸡", description: "明亮黄、背包蓝、快乐守边", primary:"#ef941e", background:"#fff8df", sidebar:"#ffedbd", card:"#fffdf5", text:"#4b3214", darkPrimary:"#ffbf55", darkBackground:"#231a0c", darkSidebar:"#30230f", darkCard:"#3b2a12", darkText:"#fff4cf", radius:24, asset:"/themes/xoyo-chicken-scene.svg", brandAsset:"/themes/xoyo-chicken-logo.svg", hangerAsset:"/themes/xoyo-chicken-hanger.png", backgroundScope:"content", backgroundFit:"cover", backgroundPosition:"right", backgroundOverlay:.68, backgroundBlur:0, composerDecorationEnabled:true, composerDecorationSource:"hanger", composerAnchor:"right-top", composerConversationAnchor:"right-top", composerScale:.21, composerOverlap:.08, decorationOpacity:.99 },
  { styleId: "jx3_swordsman", series: "游戏 IP 概念", name: "剑网3·剑影江湖", brandName:"剑侠", description: "水墨蓝灰、银发剑影、江湖留白", primary:"#537fa9", background:"#f1f5f7", sidebar:"#e0e8ed", card:"#fbfdfe", text:"#1f2c36", darkPrimary:"#89b7df", darkBackground:"#0e151b", darkSidebar:"#141f28", darkCard:"#1d2b36", darkText:"#edf5fa", radius:16, asset:"/themes/jx3-swordsman-scene.svg", brandAsset:"/themes/jx3-swordsman-logo.svg", hangerAsset:"/themes/jx3-swordsman-hanger.png", backgroundScope:"content", backgroundFit:"cover", backgroundPosition:"right", backgroundOverlay:.72, backgroundBlur:0, composerDecorationEnabled:true, composerDecorationSource:"hanger", composerAnchor:"right-top", composerConversationAnchor:"right-top", composerScale:.24, composerOverlap:.08, decorationOpacity:.98 }
];
...
const state = { brandImage: "", decorationImage: "", composerImage:"", decorationMode: "wallpaper", styleId: "claude", composerConversationAnchor:"", connected: false, platform: "", sourceImage: null, darkPalette:null, previewMode:matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light" };
...
async function applyPreset(preset, button) {
  ...
  state.composerConversationAnchor = preset.composerConversationAnchor || "";
...
function currentTheme() {
  const light = {
    ...
    composerConversationAnchor: state.composerConversationAnchor,
...
function setTheme(theme) {
  ...
  state.composerConversationAnchor = theme.composerConversationAnchor || "";
...
async function selectLingxiProgram(){const b=$("#selectLingxiButton");setBusy(b,true,"请选择灵犀程序…");showNotice("请选择灵犀独立客户端，或直接选择桌面的灵犀快捷方式；不要选择 WPS Office 内置插件。",true);try{const r=await api("/api/select-lingxi",{method:"POST",body:"{}"});renderStatus(r.status);showNotice(`已记住 ${r.executableName}，正在连接灵犀。`,true);await restartAndConnect();}catch(e){showNotice(`选择失败：${e.message}`);}finally{setBusy(b,false);}}
...
document.querySelectorAll("#backgroundScope,#backgroundFit,#backgroundPosition,#composerDecorationSource").forEach(element=>element.addEventListener("change",updatePreview));
$("#composerAnchor").addEventListener("change",()=>{ state.composerConversationAnchor=""; updatePreview(); });
...
$("#selectLingxiButton").addEventListener("click",selectLingxiProgram);
