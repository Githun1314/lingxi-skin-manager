...
export function isWindowsSkinManagerExecutable(file) {
  if (!file) return false;
  const basename = path.win32.basename(file).toLowerCase();
  return /(?:lingxi|wps)[-_. ]*skin[-_. ]*manager/.test(basename)
    || basename.includes("灵犀皮肤管理器")
    || basename.includes("皮肤管理器");
}

function normalizedWindowsPath(file) {
...
function isLikelyLingxiExecutable(file) {
  ...
  if (isWindowsSkinManagerExecutable(file)) return false;
  const basename = path.win32.basename(file).toLowerCase();
  if (/(?:setup|install|unins|uninstall|update|updater|crashpad|安装|卸载|更新)/.test(basename)) return false;
  return WINDOWS_LINGXI_EXECUTABLE_NAMES.some(name => name.toLowerCase() === basename)
    || basename.includes("lingxi")
    || basename.includes("灵犀");
}

async function executableFromCandidate(candidate, fsApi = fs, excludedFiles = new Set()) {
  if (!candidate || !await fileExists(candidate, fsApi)) return "";
  if (isWindowsOfficeLingxiPlugin(candidate)) return "";
  if (excludedFiles.has(normalizedWindowsPath(candidate))) return "";
  if (isLikelyLingxiExecutable(candidate)) return candidate;
  return "";
}

export async function findExecutableUnderRoot(root, fsApi = fs, maxDepth = 3, options = {}) {
  if (!root) return "";
  const excludedFiles = new Set((options.excludeFiles || []).map(normalizedWindowsPath));

  for (const executableName of WINDOWS_LINGXI_EXECUTABLE_NAMES) {
    const candidate = await executableFromCandidate(path.win32.join(root, executableName), fsApi, excludedFiles);
  ...
  while (queue.length) {
    ...
    const likelyFiles = entries
      .filter(entry => entry.isFile?.())
      .sort((a, b) => a.name.localeCompare(b.name));
    for (const entry of likelyFiles) {
      const candidate = await executableFromCandidate(
        path.win32.join(directory, entry.name),
        fsApi,
        excludedFiles
      );
      if (candidate) return candidate;
    ...
    for (const entry of directories) {
      ...
      for (const executableName of WINDOWS_LINGXI_EXECUTABLE_NAMES) {
        const candidate = await executableFromCandidate(path.win32.join(nestedRoot, executableName), fsApi, excludedFiles);
        if (candidate) return candidate;
      }
...
export async function findWindowsLingxiExecutable(env = process.env, fsApi = fs, options = {}) {
  ...
  if (explicit) {
    ...
    const explicitFile = await executableFromCandidate(explicit, fsApi, excludedFiles);
    if (explicitFile) return explicitFile;
    const explicitNested = await findExecutableUnderRoot(explicit, fsApi, 3, options);
  ...
  for (const hint of options.candidateFiles || []) {
    ...
    const candidate = await executableFromCandidate(hint, fsApi, excludedFiles);
    if (candidate) return candidate;
    const nested = await findExecutableUnderRoot(hint, fsApi, 2, options);
    if (nested) return nested;
  }

  for (const root of windowsLingxiRoots(env)) {
    const candidate = await findExecutableUnderRoot(root, fsApi, 3, options);
    if (candidate) return candidate;
  }
