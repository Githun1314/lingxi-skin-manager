...
升级新版时可以直接双击新的 EXE。新版会识别并接管仍在后台运行的旧管理器，页面左上角会显示当前版本号。
...
Windows 版只连接 WPS 灵犀独立客户端，不会连接 WPS Office 安装目录中的 `wpslingxi.exe` 插件。管理器会从常见安装目录、运行进程、注册表、桌面快捷方式和开始菜单快捷方式中自动寻找客户端。

如果仍未识别到，点击管理页面中的“找不到客户端？选择灵犀程序”，可以直接选择灵犀的桌面快捷方式或独立客户端程序。管理器会在本机记住该位置，之后无需重复选择。
